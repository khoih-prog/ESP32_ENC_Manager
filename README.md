...
* [Why do we need this ESP32_ENC_Manager library](#why-do-we-need-this-ESP32_ENC_Manager-library)
  * [Features](#features)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  ...
  * [2. ConfigOnSwichFS using LittleFS on ESP32_DEV with ESP32_ENC28J60](#2-ConfigOnSwichFS-using-LittleFS-on-ESP32_DEV-with-ESP32_ENC28J60)
  * [3. ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ESP32_ENC28J60](#3-ESP32_FSWebServer_DRD-using-LittleFS-on-ESP32_DEV-with-ESP32_ENC28J60)
* [Debug](#debug)
...
### Why do we need this [ESP32_ENC_Manager library](https://github.com/khoih-prog/ESP32_ENC_Manager)

#### Features

This is an `ESP32 + LwIP ENC28J60` Credentials and Connection Manager with fallback Web ConfigPortal. This Library is used for configuring ESP32 Ethernet Static / DHCP and Credentials at runtime. You can specify static DNS servers, personalized HostName and `CORS` feature.
...
This [**ESP32_ENC_Manager** library](https://github.com/khoih-prog/ESP32_ENC_Manager) currently supports these following boards:

 1. **ESP32_DEV** using `LwIP ENC28J60 Ethernet`


<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_ENC_Manager/raw/main/Images/ENC28J60.png">
</p>
...
## Prerequisites
 ...
 4. [`WebServer_ESP32_ENC library v1.5.1+`](https://github.com/khoih-prog/WebServer_ESP32_ENC) if necessary to use ESP32 boards using LwIP ENC28J60 Ethernet. To install, check [![arduino-library-badge](https://www.ardu-badge.com/badge/WebServer_ESP32_ENC.svg?)](https://www.ardu-badge.com/WebServer_ESP32_ENC)
...
The best and easiest way is to use `Arduino Library Manager`. Search for `ESP32_ENC_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/ESP32_ENC_Manager.svg?)](https://www.ardu-badge.com/ESP32_ENC_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [ESP32_ENC_Manager](https://github.com/khoih-prog/ESP32_ENC_Manager) page.
2. Download the latest release `ESP32_ENC_Manager-main.zip`.
3. Extract the zip file to `ESP32_ENC_Manager-main` directory 
4. Copy the whole `ESP32_ENC_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**ESP32_ENC_Manager** library](https://registry.platformio.org/libraries/khoih-prog/ESP32_ENC_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/ESP32_ENC_Manager/installation). Search for **ESP32_ENC_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/ESP32_ENC_Manager/issues/28) 
...
```cpp
#include <ESP32_ENC_Manager.hpp>               //https://github.com/khoih-prog/ESP32_ENC_Manager
```

in many files. But be sure to use the following `#include <ESP32_ENC_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error

```cpp
// To be included only in main(), .ino with setup() to avoid `Multiple Definitions` Linker Error
#include <ESP32_ENC_Manager.h>          //https://github.com/khoih-prog/ESP32_ENC_Manager
```

Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/ESP32_ENC_Manager/discussions/80)
...
// Must be placed before #include <ESP32_ENC_Manager.h>
...
///////////////////////////////////////////
/******************************************
   // Defined in ESP32_ENC_Manager.hpp
  typedef struct
  {
...
ESP32_ENC_manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
String tempTZ = ESP32_ENC_manager.getTimezoneName();
```
...
```cpp
const char * TZ_Result = ESP32_ENC_manager.getTZ(_timezoneName);
```
...
```cpp
ESP32_ENC_Manager ESP32_ENC_manager("Personalized-HostName");
```

then later call

```cpp
ESP32_ENC_manager.startConfigPortal()
```
...
These illustrating steps is based on the example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_ENC_Manager/tree/main/examples/ConfigOnSwitchFS)
...
The example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_ENC_Manager/tree/main/examples/ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `ESP32_ENC_Manager` class initialization to create `ESP32_ENC_Manager` object.
...
// is configuration portal requested?
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP32_ENC_Manager ESP32_ENC_manager("ConfigOnSwitchFS");
...
Adding those `ESP32_EMParameter` objects created in Step 2 using the function `addParameter()` of object `ESP32_ENC_Manager`
...
ESP32_ENC_manager.addParameter(&p_thingspeakApiKey);
ESP32_ENC_manager.addParameter(&p_sensorDht22);
ESP32_ENC_manager.addParameter(&p_pinSda);
ESP32_ENC_manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `ESP32_ENC_Manager` object will save the user input data into related `ESP32_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_ENC_Manager/raw/main/Images/Saved.png">
</p>
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    else
    {
      ...
      ESP32_ENC_manager.setConfigPortalTimeout(0);
    ...
    if (!ESP32_ENC_manager.startConfigPortal())
    ...
    const char * TZ_Result = ESP32_ENC_manager.getTZ(Ethconfig.TZ_Name);
    ...
    ESP32_ENC_manager.getSTAStaticIPConfig(EthSTA_IPconfig);
...
```cpp
ESP32_ENC_manager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
